...
class OrgFile extends PureComponent {
  ...
  handleTimestampChange(popupData) {
    if (!!popupData.get("timestampId")) {
      return (newTimestamp) =>
        this.props.org.updateTimestampWithId(
          popupData.get("timestampId"),
          newTimestamp,
        );
    ...
    switch (activePopupType) {
      ...
      case "description-editor":
        return (descriptionValue) =>
          this.handleDescriptionPopupClose(descriptionValue);
      default:
        return () => {};
    }
  }
    ...
    switch (activePopupType) {
      case "sync-confirmation":
        return (
          <SyncConfirmationModal
            ...
            onPull={() =>
              this.handleSyncConfirmationPull(activePopupData.get("path"))
            }
            onPush={() =>
              this.handleSyncConfirmationPush(activePopupData.get("path"))
            }
      ...
      case "capture":
        const template = captureTemplates.find(
          (template) =>
            template.get("id") === activePopupData.get("templateId"),
        );
      ...
      case "deadline-editor":
        ...
        if (activePopupData.get("timestampId")) {
          editingTimestamp = timestampWithId(
            headers,
            activePopupData.get("timestampId"),
          );
        } else if (activePopupData.get("logEntryIndex") !== undefined) {
          editingTimestamp = fromJS({
            firstTimestamp: headerWithId(
              headers,
              activePopupData.get("headerId"),
            ).getIn([
              "logBookEntries",
  ...
  render() {
    ...
    if (!!orgFileErrorMessage) {
      return (
        <div className="error-message-container">{orgFileErrorMessage}</div>
      );
    }
    ...
    const handlers = {
      ...
      advanceTodo: this.checkPopupAndHeader(
        preventDefault(this.handleAdvanceTodoHotKey),
      ),
      editTitle: this.checkPopupAndHeader(
        preventDefault(this.handleEditTitleHotKey),
      ),
      editDescription: this.checkPopupAndHeader(
        preventDefault(this.handleEditDescriptionHotKey),
      ),
      exitEditMode: preventDefault(this.handleExitEditModeHotKey),
      addHeader: this.checkPopupAndHeader(
        preventDefault(this.handleAddHeaderHotKey),
      ),
      removeHeader: this.checkPopupAndHeader(
        preventDefault(this.handleRemoveHeaderHotKey, true),
      ),
      moveHeaderUp: this.checkPopupAndHeader(
        preventDefault(this.handleMoveHeaderUpHotKey),
      ),
      moveHeaderDown: this.checkPopupAndHeader(
        preventDefault(this.handleMoveHeaderDownHotKey),
      ),
      moveHeaderLeft: this.checkPopupAndHeader(
        preventDefault(this.handleMoveHeaderLeftHotKey),
      ),
      moveHeaderRight: this.checkPopupAndHeader(
        preventDefault(this.handleMoveHeaderRightHotKey),
      ),
              ...
              {!!parsingErrorMessage ? (
                ...
                <Fragment>
                  {noHeadlineButContent() ? (
                    <>
                      <p>
                        Yes, your file has content. Do not worry, it is still
                        there!{" "}
                      </p>
                      <p>
                  ...
                  <p>
                    <button
                      className="btn"
                      onClick={this.handleCreateFirstHeader}
                    >
                      Create headline
                    </button>
                  </p>
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    selectedHeader:
      headers &&
      headers.find((header) => header.get("id") === selectedHeaderId),
    ...
    captureTemplates: state.capture
      .get("captureTemplates")
