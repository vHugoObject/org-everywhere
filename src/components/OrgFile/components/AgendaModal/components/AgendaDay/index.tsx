...
export default class AgendaDay extends PureComponent {
    ...
    return (
      <div className="agenda-day__container">
        <div className="agenda-day__title">
          ...
          <div className="agenda-day__title__day-name">
            {format(date, "eeee")}
          </div>
        ...
        <div className="agenda-day__headers-container">
          <div>
            {planningItemsAndHeaders.map(([planningItem, header]) => {
              const planningItemDate = dateForTimestamp(
                planningItem.get("timestamp"),
              );
              const hasTodoKeyword = !!header.getIn([
                "titleLine",
                "todoKeyword",
              ]);
                ...
                >
                  <div className="agenda-day__header__planning-item-container">
                    ...
                    <div
                      ...
                      onClick={onToggleDateDisplayType}
                    >
  ...
  getPlanningItemsAndHeaders({
    ...
    return headers
      .flatMap((header) => {
        const planningItemsforDate = header
          .get("planningItems")
          .filter((planningItem) => {
            ...
            const isCompletedTodo =
              todoKeyword &&
              isTodoKeywordCompleted(
                todoKeywordSets.get(header.get("path")),
                todoKeyword,
              );
            if (isCompletedTodo) {
              return false;
            ...
            switch (planningItem.get("type")) {
              case "DEADLINE":
                if (isToday(date)) {
                  ...
                  return isWithinInterval(planningItemDate, {
                    start: dateStart,
                    end: dateEnd,
                  });
                }
              case "SCHEDULED":
                ...
                if (timestamp.get("delayType")) {
                  const hasBeenRepeated = header
                    ...
                    .some(
                      (propertyListItem) =>
                        propertyListItem.get("property") === "LAST_REPEAT",
                    );
                ...
                return isWithinInterval(appearDate, {
                  start: dateStart,
                  end: dateEnd,
                });
              default:
        ...
        return planningItemsforDate.map((planningItem) => [
          planningItem,
          header,
        ]);
      })
      .sortBy(([planningItem]) => {
        const { startHour, startMinute, endHour, endMinute, month, day } =
          planningItem.get("timestamp").toJS();
        return [
          startHour ? 0 : 1,
          startHour,
          startMinute,
