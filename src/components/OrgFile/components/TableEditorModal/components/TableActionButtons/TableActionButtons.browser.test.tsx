...
const testTableActionsRenderer = async (): Promise<
  [
...
> => {
  ...
  return [
    testSetupObj,
    testStore,
    [testTableTotalRows, testTableTotalColumns],
    [testRandomRowIndex, testRandomColumnIndex],
  ];
};
...
describe("TableCell tests", async () => {
  ...
  describe("Column Tests", async () => {
    test("Test add-column-button", async () => {
      const [{ screen, user }, testStore, [, testTableTotalColumns]] =
        await testTableActionsRenderer();
  ...
  describe("Row Tests", async () => {
    test("Test add-row-button", async () => {
      const [{ screen, user }, testStore, [testTableTotalRows]] =
        await testTableActionsRenderer();
  ...
  describe("Movement Tests", async () => {
    test("Test up-button", async () => {
      ...
      const actualTableBeforeMove: MapOf<OrgTable> = getSelectedTable(
        testStore.getState(),
      );
      const actualRowBeforeMove: List<MapOf<OrgTableCell>> =
        getContentsOfTableRow(testRandomRowIndex, actualTableBeforeMove);
      const expectedRowIndexAfterMove: number = clampSubtractFromZero(
        testTableTotalRows,
        testRandomRowIndex,
        1,
      );
      ...
      const actualTable = getSelectedTable(testStore.getState());
      const actualRowAfterMove: List<MapOf<OrgTableCell>> =
        getContentsOfTableRow(expectedRowIndexAfterMove, actualTable);
    ...
    test("Test left-button", async () => {
      ...
      const actualColumnBeforeMove = getContentsOfTableColumn(
        testRandomColumnIndex,
        actualTableBeforeMove,
      );

      const expectedColumnIndexAfterMove = clampSubtractFromZero(
        testTableTotalColumns,
        testRandomColumnIndex,
        1,
      );
    ...
    test("Test right-button", async () => {
      ...
      const expectedColumnIndexAfterMove = clampAddFromZero(
        testTableTotalColumns,
        testRandomColumnIndex,
        1,
      );
    ...
    test("Test down-button", async () => {
      ...
      const actualTableBeforeMove = getSelectedTable(testStore.getState());
      const actualRowBeforeMove = getContentsOfTableRow(
        testRandomRowIndex,
        actualTableBeforeMove,
      );

      const expectedRowIndexAfterMove = clampAddFromZero(
        testTableTotalRows,
        testRandomRowIndex,
        1,
      );
