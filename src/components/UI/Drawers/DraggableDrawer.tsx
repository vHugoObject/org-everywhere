...
const DraggableDrawer = ({
  ...
  const handleAnimationRest = () =>
    !isVisible && !!onClose ? onClose() : void 0;

  const handleTouchStart = (event) =>
    (initialClientY.current = event.targetTouches[0].clientY);
  ...
  useEffect(() => {
    ...
    if (!!innerContainer.current) {
      ...
      innerContainer.current.addEventListener(
        "touchend",
      ...
      return () => {
        innerContainerCurrent.removeEventListener(
          "touchstart",
          handleTouchStart,
        );
