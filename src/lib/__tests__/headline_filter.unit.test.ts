...
const gtags = (tags) =>
  tags.map((x) => ({ type: "tag", words: [x], exclude: false }));
const gtagsOr = (tagsArr) =>
  tagsArr.map((x) => ({ type: "tag", words: x, exclude: false }));
// Generate case-sensitive filter
const gcs = (word) => [
  { type: "case-sensitive", words: [word], exclude: false },
];
const gcss = (words) =>
  words.map((x) => ({ type: "case-sensitive", words: [x], exclude: false }));
...
const gics = (words) =>
  words.map((x) => ({ type: "ignore-case", words: [x], exclude: false }));
const gicsOr = (wordsArr) =>
  wordsArr.map((x) => ({ type: "ignore-case", words: x, exclude: false }));
// Generate property filter
const gprop = (key, word) => [
  { type: "property", property: key, words: [word], exclude: false },
];
const gprops = (props) =>
  props.map(([x, y]) => ({
    type: "property",
...
describe("Match function for headline filter", () => {
  ...
  describe("Tests for combined matching", () => {
    ...
    test("Match tag, TODO, ignore-case, and property", () => {
      const filterExpr = gtag("spec_tag").concat(
        gcs("TODO"),
        gic("spec"),
        gprop("prop1", "def"),
      );
...
describe("Computation of completions and suggestions for task filter", () => {
  ...
  describe("Computation of suggestions for datalist", () => {
    // Function under test:
    const compute = computeCompletionsForDatalist(
      todoKeywords,
      tagNames,
      allProperties,
    );
