...
export const convertColorInstanceIntoCSS = Function.prototype.call.bind(
  Color.prototype.string,
);
export const createColorObject = pipe([
  Array.of,
...
export const interpolateColors = (
  ...
  const alpha: number =
    colorB.alpha && colorA.alpha
      ? (colorB.alpha - colorA.alpha) * interpolationFactor + colorA.alpha
      : 0;
  return {
...
export const getThemeFromThemeObject = curry(
  (theme: string, colorScheme: string): Record<string, string> => {
    return property([theme, colorScheme], themes);
  },
);

export const loadTheme = (
  theme: string = "Solarized",
