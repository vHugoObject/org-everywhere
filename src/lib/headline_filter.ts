...
export const timeFilter = (filterDescription) => {
  ...
  if (timeFilterDescription.type === "point") {
    ...
    return (timestamp) =>
      lower <= dateForTimestamp(timestamp) &&
      dateForTimestamp(timestamp) <= upper;
    ...
    if (isRelative(from) === isRelative(to)) {
      ...
      upper = resolveTo(to);
      lower = subtractTimestampUnitFromDate(
        new Date(upper),
        from.value,
        from.unit,
      );
...
const orChainDate = (dates) => (filter) =>
  dates.size !== 0 && dates.some((x) => filter(x));

export const isMatch = (filterExpr) => {
  const filterFilter = (type, exclude) => (x) =>
    x.type === type && x.exclude === exclude;
  ...
  const filterCS = filterExpr
    .filter(filterFilter("case-sensitive", false))
    .map(words);
  const filterIC = filterExpr
    .filter(filterFilter("ignore-case", false))
    .map(wordsLowerCase);
  ...
  const filterDate = filterField
    .filter((f) => f.field.type === "date")
    .map(timeFilter);
  //const filterClock = filterField.filter((f) => f.field.type === 'clock').map(timeFilter);
  const filterSchedule = filterField
    .filter((f) => f.field.type === "scheduled")
    .map(timeFilter);
  const filterDeadline = filterField
    .filter((f) => f.field.type === "deadline")
    .map(timeFilter);

  const filterTagsExcl = filterExpr
    .filter(filterFilter("tag", true))
    .map(words);
  const filterCSExcl = filterExpr
    .filter(filterFilter("case-sensitive", true))
    .map(words);
  const filterICExcl = filterExpr
    .filter(filterFilter("ignore-case", true))
    .map(wordsLowerCase);
  ...
  const filterDesc = filterField
    .filter(
      (f) =>
        f.field.type === "description" && f.field.text.type === "ignore-case",
    )
  ...
  const filterDescCS = filterField
    .filter(
      (f) =>
        ...
        f.field.text.type === "case-sensitive",
    )
  ...
  return (header) => {
    ...
    const properties = header
      ...
      .map((p) => [
        p.get("property"),
        attributedStringToRawText(p.get("value")),
      ]);
    ...
    if (filterDesc.length > 0 || filterDescCS.length > 0) {
      const stripMarkup = (text) =>
        text.replace(/\*([\w]*)\*/, (match, p1, _, __) => p1);
...
export const computeCompletions =
  (todoKeywords, tagNames, allProperties) =>
  (filterExpr, filterString, curserPosition) => {
    ...
    const charTwoBeforeCursor =
      curserPosition > 1 ? filterString.charAt(curserPosition - 2) : "";
    ...
    if (logicalCursorPosition === null) {
      ...
      if (charBeforeCursor === "-" && [" ", ""].includes(charTwoBeforeCursor))
        return todoKeywords;
      return [];
      ...
      if (charBeforeCursor === ":") {
        if (charTwoBeforeCursor === " " || charTwoBeforeCursor === "")
          ...
          const indexOfOtherColon = filterString
            .substring(0, curserPosition - 1)
            .lastIndexOf(":");
          ...
          if (indexOfOtherColon >= 0 && maybePropertyName.match(/^[^ ]+$/)) {
            ...
            return computeAllPropertyValuesFor(
              fromJS(allProperties),
              maybePropertyName,
            )
    ...
    if (charBeforeCursor === ":") {
      ...
      const indexOfOtherColon = filterString
        .substring(0, curserPosition)
        .lastIndexOf(":");
      const maybeTagName = filterString.substring(
        indexOfOtherColon + 1,
        curserPosition - 1,
      );
...
export const computeCompletionsForDatalist =
  ...
  (filterExpr, filterString, curserPosition) => {
    ...
    return completions.map(
      (x) =>
        filterString.substring(0, curserPosition) +
        x +
        filterString.substring(curserPosition),
    );
  };
...
const computeLogicalPosition = (filterExpr, filterString, curserPosition) => {
  ...
  if (curserPosition > filterExpr[filterExpr.length - 1].endOffset)
    return SPACE_SURROUNDED;
  return value;
};
