...
describe("org reducer", () => {
  ...
  function check_is_undoable(state, action: OrgAction) {
    ...
    const firstHeader = state.org.present
  ...
  function assertElementDidNotChangeForHeaderIds(
    element,
    oldHeaders,
    newHeaders,
    headerIds,
  ...
  const curriedAssertElementDidNotChangeForHeaderIds = curry(
    assertElementDidNotChangeForHeaderIds,
  );
  ...
  describe("REFILE_SUBTREE", () => {
    ...
    it("should handle REFILE_SUBTREE", () => {
      ...
      expect(
        ...
        ["A header with [[https://org-everywhere.org][a link]]", 1],
      ...
      const action = types.refileSubtree(
        path,
        sourceHeaderId,
        path,
        targetHeaderId,
      ...
      expect(
        extractTitlesAndNestings(newState.getIn(["files", path, "headers"])),
      ).toEqual([
        ["Top level header", 1],
        ["A nested header", 2],
        ["PROJECT Foo", 3],
  ...
  describe("INSERT_CAPTURE", () => {
    ...
    function expectOrigFirstHeader(headers) {
      expect(extractTitleAndNesting(headers.first())).toEqual([
        "Top level header",
        1,
      ]);
    }
  ...
  describe("header tree", () => {
    ...
    describe("MOVE_HEADER_DOWN", () => {
      ...
      it("should just dirty if already on the bottom", () => {
        check_just_dirtying(
          state.org.present,
          types.moveHeaderDown(nestedHeader2Id),
        );
      });
    ...
    describe("MOVE_HEADER_UP", () => {
      ...
      it("should just dirty if already at the top", () => {
        check_just_dirtying(
          state.org.present,
          types.moveHeaderUp(nestedHeaderId),
        );
      });
    ...
    describe("MOVE_SUBTREE_LEFT", () => {
      ...
      it("should just dirty when trying to move left a toplevel subtree", () => {
        check_just_dirtying(
          state.org.present,
          types.moveSubtreeLeft(topLevelHeaderId),
        );
      });
    ...
    describe("REMOVE_HEADER", () => {
      ...
      it("should reset header narrowing", () => {
        const narrowedState = reducer(
          state.org.present,
          types.narrowHeader(nestedHeaderId),
        );
        expect(
          narrowedState.getIn(["files", path, "narrowedHeaderId"]),
        ).toEqual(nestedHeaderId);
        const newState = reducer(
          narrowedState,
          types.removeHeader(nestedHeaderId),
        );
        expect(newState.getIn(["files", path, "narrowedHeaderId"])).toEqual(
          null,
        );
      });
    ...
    describe("ADD_HEADER", () => {
      it("should handle ADD_HEADER and widen", () => {
        ...
        expect(
          extractTitlesAndNestings(oldState.getIn(["files", path, "headers"])),
        ).toEqual([
          ["Top level header", 1],
          ["A nested header", 2],
          ["A deep nested header", 3],
        ...
        const stateSelected = reducer(
          oldState,
          types.narrowHeader(nestedHeaderId),
        );
        expect(
          stateSelected.getIn(["files", path, "narrowedHeaderId"]),
        ).toEqual(nestedHeaderId);
        const newState = reducer(
          stateSelected,
          types.addHeader(nestedHeaderId),
        );
    ...
    describe("selecting ", () => {
      ...
      function openHeaders(state, opennessState) {
        return reducer(
          state.set("opennessState", opennessState),
          types.applyOpennessState(path),
        );
      }
      ...
      describe("SELECT_PREVIOUS_VISIBLE_HEADER", () => {
        it("should skip invisible header", () => {
          ...
          expect(
            stateSelected.getIn(["files", path, "selectedHeaderId"]),
          ).toEqual(nestedHeader2Id);
          const newState = reducer(
            stateSelected,
            types.selectPreviousVisibleHeader(),
          );
          expect(newState.getIn(["files", path, "selectedHeaderId"])).toEqual(
            nestedHeaderId,
          );
        });
      ...
      describe("SELECT_NEXT_VISIBLE_HEADER", () => {
        it("start from the first", () => {
          expect(
            state.org.present.getIn(["files", path, "selectedHeaderId"]),
          ).toBeUndefined();
          const newState = reducer(
            state.org.present,
            types.selectNextVisibleHeader(),
          );
          expect(newState.getIn(["files", path, "selectedHeaderId"])).toEqual(
            topLevelHeaderId,
          );
      ...
      describe("SELECT_NEXT_SIBLING_HEADER", () => {
        it("ignore when on the last sibling", () => {
          ...
          const newState = reducer(
            oldState,
            types.selectNextSiblingHeader(nestedHeader2Id),
          );
          expect(newState.getIn(["files", path, "selectedHeaderId"])).toEqual(
            topLevelHeaderId,
          );
          ...
          expect(newState2.getIn(["files", path, "selectedHeaderId"])).toEqual(
            nestedHeader2Id,
          );
        });
      });
    ...
    describe("UPDATE_HEADER_DESCRIPTION", () => {
      ...
      it("should handle UPDATE_HEADER_DESCRIPTION", () => {
        const action = types.updateHeaderDescription(
          nestedHeaderId,
          newDescription,
        );
        const newState = reducer(state.org.present, action);
      ...
      it("is undoable", () => {
        check_is_undoable(
          state,
          types.updateHeaderDescription(nestedHeaderId, newDescription),
        );
      });
    });
    ...
    describe("TOGGLE_HEADER_OPENED", () => {
      ...
      it("should close the header and subheaders on the second toggle", () => {
        ...
        const nestedOpen = reducer(
          topLevelOpen,
          types.toggleHeaderOpened(nestedHeaderId, true),
        );
        ...
        expect(
          headerWithId(
            reopened.getIn(["files", path, "headers"]),
            topLevelHeaderId,
          ).get("opened"),
        ).toEqual(true);
        expect(
      ...
      it("should ignore if narrowed and open", () => {
        ...
        const narrowedState = reducer(
          openState,
          types.narrowHeader(topLevelHeaderId),
        );
        const newState = reducer(
          narrowedState,
          types.toggleHeaderOpened(topLevelHeaderId, true),
  ...
  describe("ADVANCE_TODO_STATE", () => {
    ...
    beforeEach(() => {
      ...
      doneHeaderId = state.org.present
        .getIn(["files", path, "headers"])
        .get(0)
        .get("id");
      todoHeaderId = state.org.present
        .getIn(["files", path, "headers"])
        .get(1)
        .get("id");
      regularHeaderId = state.org.present
        .getIn(["files", path, "headers"])
        .get(2)
        .get("id");
      activeTimestampWithRepeaterHeaderId = state.org.present
        ...
        .get(3)
        .get("id");
      repeatingHeaderId = state.org.present
        .getIn(["files", path, "headers"])
        .get(4)
        .get("id");
      notDoneId = state.org.present
        .getIn(["files", path, "headers"])
        .get(5)
        .get("id");
      notDoneWithLogsId = state.org.present
        .getIn(["files", path, "headers"])
        .get(6)
        .get("id");
      allTestHeaderIds = [
    ...
    function check_todo_keyword_kept(oldHeaders, newHeaders, headerId) {
      expect(
        headerWithId(oldHeaders, headerId).getIn(["titleLine", "todoKeyword"]),
      ).toEqual(
    ...
    function check_header_kept(oldHeaders, newHeaders, headerId) {
      expect(headerWithId(oldHeaders, headerId)).toEqual(
        headerWithId(newHeaders, headerId),
      );
    }
    ...
    it("should advance TODO state", () => {
      ...
      const newHeaders = reducer(
        state.org.present,
        types.advanceTodoState(todoHeaderId),
      ).getIn(["files", path, "headers"]);
      ...
      expect(extractTitlesAndNestings(oldHeaders)).toEqual(
        extractTitlesAndNestings(newHeaders),
      );
    });
    ...
    it("should advance DONE state", () => {
      ...
      const newHeaders = reducer(
        state.org.present,
        types.advanceTodoState(doneHeaderId),
      ).getIn(["files", path, "headers"]);
      check_header_kept(oldHeaders, newHeaders, regularHeaderId);
    ...
    it("should advance non-TODO state", () => {
      ...
      const newHeaders = reducer(
        state.org.present,
        types.advanceTodoState(regularHeaderId),
      ).getIn(["files", path, "headers"]);
    ...
    it("should advance repeating task", () => {
      ...
      assertDescriptionDidNotChangeForHeaderIds(
        oldHeaders,
        newHeaders,
        allTestHeaderIds,
      );
      ...
      assertLogNotesDidNotChangeForHeaderIds(
        oldHeaders,
        newHeaders,
        unchangedHeaders,
      );
      assertPlanningItemsDidNotChangeForHeaderIds(
        oldHeaders,
        newHeaders,
        unchangedHeaders,
      );
      ...
      expect(
        headerWithId(newHeaders, repeatingHeaderId).get("logNotes").size,
      ).toBeGreaterThan(
        headerWithId(oldHeaders, repeatingHeaderId).get("logNotes").size,
      );
      expect(
        headerWithId(newHeaders, repeatingHeaderId).get("planningItems"),
      ).not.toEqual(
    ...
    it("should advance repeating task again", () => {
      ...
      expect(
        headerWithId(newHeaders, repeatingHeaderId).get("description").size,
      ).toEqual(
      ...
      // The nesting levels remain intact.
      expect(extractTitlesAndNestings(intermHeaders)).toEqual(
        extractTitlesAndNestings(newHeaders),
      );
    });
    ...
    it("should advance active timestamp with repeater in header", () => {
      ...
      expect(
        headerWithId(newHeaders, activeTimestampWithRepeaterHeaderId).get(
          "logNotes",
        ).size,
      ).toBeGreaterThan(
        headerWithId(oldHeaders, activeTimestampWithRepeaterHeaderId).get(
          "logNotes",
        ).size,
      );
  ...
  describe("UPDATE_LOG_ENTRY_TIME", () => {
    ...
    beforeEach(() => {
      state = setUpStateForFile(path, testOrgFile);
      headerId = state.org.present
        .getIn(["files", path, "headers"])
        .get(0)
        .get("id");
      irrelevantHeaderId = state.org.present
        .getIn(["files", path, "headers"])
        .get(1)
        .get("id");
    });
    ...
    it("should handle UPDATE_LOG_ENTRY_TIME", () => {
      ...
      check_kept((st) =>
        headerWithId(st.getIn(["files", path, "headers"]), irrelevantHeaderId),
      );
      check_kept((st) =>
      ...
      check_kept(
        (st) =>
          headerWithId(st.getIn(["files", path, "headers"]), headerId).get(
            "logBookEntries",
          ).size,
      );
      check_kept((st) =>
  ...
  describe("SET_ORG_FILE_ERROR_MESSAGE", () => {
    ...
    it("should handle SET_ORG_FILE_ERROR_MESSAGE", () => {
      const newState = reducer(
        state.org.present,
        types.setOrgFileErrorMessage(message),
      );
  ...
  describe("UPDATE_PROPERTY_LIST_ITEMS", () => {
    ...
    it("should handle UPDATE_PROPERTY_LIST_ITEMS", () => {
      ...
      expect(
        headerWithId(newState.getIn(["files", path, "headers"]), headerId).get(
          "propertyListItems",
        ),
      ).toEqual(properties);
  ...
  describe("ADD_NEW_PLANNING_ITEM", () => {
    ...
    it("should handle ADD_NEW_PLANNING_ITEM", () => {
      const newState = reducer(
        state.org.present,
        types.addNewPlanningItem(headerId, "DEADLINE"),
      );
      expect(
        headerWithId(newState.getIn(["files", path, "headers"]), headerId).get(
          "planningItems",
  ...
  describe("UPDATE_TIMESTAMP_WITH_ID", () => {
    ...
    beforeEach(() => {
      ...
      bodyTsId = state.org.present.getIn([
        "files",
        path,
        "headers",
        0,
    ...
    it("should just dirty when trying to update invalid id", () => {
      check_just_dirtying(
        state.org.present,
        types.updateTimestampWithId(invalidId, "dummy"),
      );
    });
  });
  ...
  describe("REORDER_TAGS", () => {
    ...
    it("should handle REORDER_TAGS", () => {
      const stateSelected = reducer(state.org.present, {
        type: "SELECT_HEADER",
        headerId,
      });
      const newState = reducer(
        stateSelected,
        types.reorderTags(fromIndex, toIndex),
      );
    ...
    it("should just dirty when working with invalid header id", () => {
      const selectedState = reducer(state.org.present, {
        type: "SELECT_HEADER",
        invalidId,
      });
  ...
  describe("SET_HEADER_TAGS", () => {
    ...
    it("should handle SET_HEADER_TAGS", () => {
      ...
      const headerId = stateInserted
        .getIn(["files", path, "headers"])
        .get(0)
        .get("id");
      const newState = reducer(
        stateInserted,
        types.setHeaderTags(headerId, tags),
      );
    ...
    it("should just dirty when working with invalid header id", () => {
      check_just_dirtying(
        state.org.present,
        types.setHeaderTags(invalidId, tags),
      );
    });
  });
  ...
  describe("ADVANCE_CHECKBOX_STATE", () => {
    ...
    beforeEach(() => {
      ...
      checkedBoxC = headerWithId(headers, topHeaderId).getIn([
        "description",
        0,
        "items",
        2,
    ...
    it("should check the box", () => {
      ...
      const newState = reducer(
        oldState,
        types.advanceCheckboxState(uncheckedBoxB),
      );
      ...
      check_kept((st) =>
        headerWithId(st.getIn(["files", path, "headers"]), bottomHeaderId),
      );
    });
    ...
    it("should uncheck the box", () => {
      ...
      const newState = reducer(
        oldState,
        types.advanceCheckboxState(checkedBoxC),
      );
    ...
    it("should just dirty when checkbox is a nest header", () => {
      check_just_dirtying(
        state.org.present,
        types.advanceCheckboxState(compoundBoxE),
      );
    });
    ...
    it("should check the parent boxes and update cookies when complete", () => {
      ...
      const newState = reducer(
        oldState,
        types.advanceCheckboxState(deepNestedBoxH),
      );
    ...
    it("should keep the partial state when some children are not checked", () => {
      ...
      const newState = reducer(
        oldState,
        types.advanceCheckboxState(bottomNestedBoxK),
      );
      expect(
        headerWithId(newState.getIn(["files", path, "headers"]), bottomHeaderId)
          .getIn(["description", 0, "items"])
    ...
    it("should clear the compound when children are undone", () => {
      ...
      const newState = reducer(
        oldState,
        types.advanceCheckboxState(bottomDeepNestedBoxN),
      );
      expect(
        headerWithId(newState.getIn(["files", path, "headers"]), bottomHeaderId)
          .getIn(["description", 0, "items"])
    ...
    function insertBugIntoCheckboxM(oldState, bug) {
      return oldState.updateIn(
        [
          ...
          "headers",
          indexOfHeaderWithId(
            oldState.getIn(["files", path, "headers"]),
            bottomHeaderId,
          ),
          "description",
          0,
  ...
  describe("table", () => {
    ...
    beforeEach(() => {
      state = setUpStateForFile(path, testOrgFile);
      cellId = firstTable(state.org.present, path).getIn([
        "contents",
        1,
        "contents",
        1,
    ...
    describe("UPDATE_TABLE_CELL_VALUE", () => {
      it("should handle UPDATE_TABLE_CELL_VALUE", () => {
        const newState = reducer(
          state.org.present,
          types.updateTableCellValue(cellId, newValue),
        );
        expect(
        ...
        const check_kept = check_kept_factory(state.org.present, newState);
        check_kept((st) =>
          st.getIn(["files", path, "headers", 0, "titleLine"]),
        );
        check_kept((st) =>
          firstTable(st, path).getIn(["contents", 0, "contents"]),
        );
        check_kept((st) =>
          firstTable(st, path).getIn(["contents", 2, "contents"]),
        );
        check_kept((st) =>
          firstTable(st, path).getIn(["contents", 1, "contents", 0]),
        );
        check_kept((st) =>
          firstTable(st, path).getIn(["contents", 1, "contents", 2]),
        );
      });
    });
    ...
    describe("MOVE_TABLE_COLUMN_RIGHT", () => {
      it("should handle MOVE_TABLE_COLUMN_RIGHT", () => {
        ...
        const newState = reducer(
          stateCellSelected,
          types.moveTableColumnRight(),
        );
        const check_kept = check_kept_factory(state.org.present, newState);

        [0, 1, 2].forEach((i) => {
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents", 1]),
          ).toEqual(
            firstTable(oldState, path).getIn(["contents", i, "contents", 2]),
          );
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents", 2]),
          ).toEqual(
            firstTable(oldState, path).getIn(["contents", i, "contents", 1]),
          );
          check_kept((st) =>
            firstTable(st, path).getIn(["contents", i, "contents", 0]),
          );
          check_kept(
            (st) =>
              firstTable(st, path).getIn(["contents", i, "contents"]).size,
          );
        });
    ...
    describe("MOVE_TABLE_COLUMN_LEFT", () => {
      it("should handle MOVE_TABLE_COLUMN_LEFT", () => {
        ...
        [0, 1, 2].forEach((i) => {
          ...
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents", 0]),
          ).toEqual(
            firstTable(oldState, path).getIn(["contents", i, "contents", 1]),
          );
          check_kept((st) =>
            firstTable(st, path).getIn(["contents", i, "contents", 2]),
          );
          check_kept(
            (st) =>
              firstTable(st, path).getIn(["contents", i, "contents"]).size,
    ...
    describe("REMOVE_TABLE_COLUMN", () => {
      it("should handle REMOVE_TABLE_COLUMN", () => {
        ...
        [0, 1, 2].forEach((i) => {
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents"]).size,
          ).toEqual(
            firstTable(oldState, path).getIn(["contents", i, "contents"]).size -
              1,
          );
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents", 1]),
    ...
    describe("ADD_NEW_TABLE_COLUMN", () => {
      it("should handle ADD_NEW_TABLE_COLUMN", () => {
        ...
        [0, 1, 2].forEach((i) => {
          ...
          expect(
            firstTable(newState, path).getIn(["contents", i, "contents", 3]),
          ).toEqual(
            firstTable(oldState, path).getIn(["contents", i, "contents", 2]),
          );
          check_kept((st) =>
          ...
          check_kept((st) =>
            firstTable(st, path).getIn(["contents", i, "contents", 1]),
          );
        });
        check_kept((st) => firstTable(st, path).get("contents").size);
      });
  ...
  describe("APPLY_OPENNESS_STATE", () => {
    ...
    it("should handle APPLY_OPENNESS_STATE", () => {
      const stateWithOpenness = state.org.present.set(
        "opennessState",
        opennessState,
      );
      const newState = reducer(
    ...
    it("should do nothing when no openness state is set for the file", () => {
      const stateWithOpenness = state.org.present.setIn(
        ["opennessState", path],
        fromJS({}),
      );
      const newState = reducer(
  ...
  describe("EXIT_EDIT_MODE", () => {
    ...
    it("should handle EXIT_EDIT_MODE", () => {
      const editState = reducer(
        state.org.present,
        types.enterEditMode("description"),
      );
      expect(editState.getIn(["files", path, "editMode"])).toEqual(
        "description",
      );
  ...
  describe("SET_SEARCH_FILTER_INFORMATION", () => {
    ...
    it("should ignore invalid search filter", () => {
      const oldState = selectHeader(state.org.present, headerId);
      const action = types.setSearchFilterInformation(
        invalidFilter,
        0,
        "refile",
      );
  ...
  describe("handle empty files", () => {
    it("creates a new first header in an empty file", () => {
      ...
      expect(
        state.org.present.getIn(["files", path, "headers"]).toJS(),
      ).toEqual([]);
    ...
    it("embedds linesBeforeHeadings into the first headline", () => {
      ...
      expect(
        state.org.present.getIn(["files", path, "linesBeforeHeadings"]).toJS(),
      ).toEqual([
      ...
      expect(
        newState.getIn(["files", path, "linesBeforeHeadings"]).toJS(),
      ).toEqual([]);
    });
  });
});
