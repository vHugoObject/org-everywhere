...
const getActionType = (node: Node): string => {
  return pipe([
    filter((descendant: PropertyAssignment): boolean => descendant.getName() == "type"),
    ([descendant]: Array<PropertyAssignment>) => descendant?.getInitializer()?.getText()
...
const collectActionValues = (sourceFile: SourceFile): Array<string> => {
  ...
  sourceFile.forEachDescendant((node: Node) => {
	...
	if (Node.isPropertyAssignment(descendant) || Node.isShorthandPropertyAssignment(descendant)) {
...
const getAllActionValues  = (filePaths: ReadonlyArray<string>): Array<string> => {
...
const runReplacers = (string: string): string => {
  return reduce((prev: string, curr: (str: string) => string): string => {
    return curr(prev)
  },
    string,
    REPLACERS)
...
const getTypeForValue = cond([
  ...
  [overSome([eq("newSettings"), eq("data")]), constant("Record<string, string>")],
  [stubTrue, constant("any")],
])


const convertPropertyAssignmentIntoType = (node: PropertyAssignment): PropertySignatureStructure  => {
...
const convertShorthandPropertyAssignmentIntoType = (node: ShorthandPropertyAssignment): PropertySignatureStructure  => {
  const name: string = node.getName()
  return {
    kind: StructureKind.PropertySignature,
    name,
...
const convertActionValueIntoWriter = (node: ObjectLiteralExpression): WriterFunction => {
...
const collectActions = (sourceFile: SourceFile): Array<ObjectLiteralExpression> => {
...
const convertActionsIntoUnionType = (actions: Array<ObjectLiteralExpression>): WriterFunction => {
  const preppedActions: Array<WriterFunction> = map(convertActionValueIntoWriter)(actions)
  const [first, second, ...rest] = preppedActions
  return Writers.unionType(first, second, ...rest)
}

const createTypesForActionsInFile = (sourceFile: SourceFile, typeName: string): TypeAliasDeclarationStructure => {
  const typesWriter: WriterFunction = pipe([collectActions, convertActionsIntoUnionType])(sourceFile);
  return {
...
const createAllActionTypes = async(filePathTuples = ACTIONTYPESTOCREATE): Promise<void> => {
  const project = new Project({});
  const typesFile = project.addSourceFileAtPath("./src/types.ts");
  forEach(([filePath, typeName]: [string, string]): void => {
    const sourceFile = project.addSourceFileAtPath(filePath);
    const variableStatementStructure: TypeAliasDeclarationStructure = createTypesForActionsInFile(sourceFile, typeName)
...
const isActionCreator = (node: FunctionDeclaration | ArrowFunction): boolean => {
  const returnValue = node.getLastChildIfKind(SyntaxKind.ParenthesizedExpression)
...
const annotateAllActionDispatcherReturnValues = async(filePathTuples = ACTIONTYPESTOCREATE): Promise<void> => {
  const project = new Project({});
  forEach(([filePath]: [string, string]): void => {
    const sourceFile = project.addSourceFileAtPath(filePath);
    sourceFile.forEachDescendant((descendant: Node): void => {
      if (Node.isFunctionDeclaration(descendant) || Node.isArrowFunction(descendant)) {
	const currentReturnType = descendant.getReturnType().getText();
	if (descendant.getReturnTypeNode()) return
	if (includes("import", currentReturnType) || eq("any", currentReturnType)) return
	currentReturnType && currentReturnType !== "typeName" && descendant.setReturnType(currentReturnType)
      }
    })
  })(filePathTuples)
  await project.save()
...
const getNamedImportsAsString = map((currentNamedImport: ImportSpecifier): string => currentNamedImport.getName())

const sourceFileHasNamedImport = (sourceFile: SourceFile, namedImport: string): boolean => {
  const sourceFileNamedImports: Set<string> = pipe([
    flatMap((importDeclaration: ImportDeclaration): Array<string> => getNamedImportsAsString(importDeclaration.getNamedImports())),
...
const getAllOrgTypesAsStrings = (): Array<string> => {
  const project = new Project({});
  const typesFile = project.addSourceFileAtPath("./src/types.ts");
  const allTypes = [...typesFile.getDescendantsOfKind(SyntaxKind.TypeAliasDeclaration), ...typesFile.getDescendantsOfKind(SyntaxKind.InterfaceDeclaration)]
  return map((x: TypeAliasDeclaration | InterfaceDeclaration): string => x.getName())(allTypes)
}
...
const mapAllNamedImportsToModuleSpecifier = (importDeclaration: ImportDeclaration): Record<string, string> => {
  const moduleSpecifier: string = importDeclaration.getModuleSpecifier().getLiteralValue();
  return pipe([
    map((x: ImportSpecifier): [string, string] => [x.getName(), moduleSpecifier]),
    Object.fromEntries
...
const importDeclarationContainsTypeImports = (importDeclaration: ImportDeclaration): boolean => {
  const imports = filter(isTypeImportSpecifier)(importDeclaration.getNamedImports())
  return imports.length > 0
}
const filterForLibraryTypeImports = (importDeclaration: ImportDeclaration): boolean => {
  if (importDeclaration.isModuleSpecifierRelative()) return false
  return importDeclaration.isTypeOnly() || importDeclarationContainsTypeImports(importDeclaration)
}

const mapAllNamedTypeImportsToModuleSpecifier = (importDeclaration: ImportDeclaration): Record<string, string> => {
  if (importDeclaration.isTypeOnly()) return mapAllNamedTypeImportsToModuleSpecifier(importDeclaration)
  const moduleSpecifier: string = importDeclaration.getModuleSpecifier().getLiteralValue();
  return pipe([
...
const createImportsMapping = (importDeclarationFilter: (importDeclaration: ImportDeclaration) => boolean,
  importMapper: (importDeclaration: ImportDeclaration) => Record<string, string>) => (): Record<string, string> => {

  const project = new Project({});
  project.addSourceFilesAtPaths("./src/**");
  const reducer = (mapping: Record<string, string>, sourceFile: SourceFile): Record<string, string> => {
    const newMaps = pipe([
...
const getAllLibraryImports = createImportsMapping(filterForLibraries, mapAllNamedImportsToModuleSpecifier)
const getAllLibraryTypeImports = createImportsMapping(filterForLibraryTypeImports, mapAllNamedImportsToModuleSpecifier)
...
const isPrimitiveType = (type: Type, typeSet: Set<string> = PRIMITIVETYPESSET): boolean => {
  ...
  if (type.getTypeArguments()) return every(isPrimitiveType, type.getTypeArguments())
  return false
}
const handleTypescriptInference = (node: ParameterDeclaration, sourceFile: SourceFile, type: Type): void => {
  //if (normalType) just return it as a string
  if (isPrimitiveType(type)) node.setType(type.getText())
}

const annotateParameter = (sourceFile: SourceFile, node: ParameterDeclaration): void => {
...
const annotateAllFunctionArguments = async(filePaths: Array<string>): Promise<void> => {
  const project = new Project({});
